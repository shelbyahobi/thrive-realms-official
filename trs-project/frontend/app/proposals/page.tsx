...
export default function ProposalsPage() {
    ...
    async function fetchProposals() {
        ...
        try {
            ...
            const CHUNK_SIZE = 2000; // conservative
            const TOTAL_SEARCH = 100000; // Search deeper history (approx 4-5 days on BSC)
    ...
    return (
        <div className="container mx-auto px-4 py-12">
            ...
            <div className="space-y-4">
                {proposals.map(p => {
                    const titleMatch = p.description.match(/^#\s+(.+)$/m);
                    const title = titleMatch ? titleMatch[1] : `Proposal #${p.id.substring(0, 6)}...`;
                    const summary = p.description.replace(/^#\s+.+$/m, '').substring(0, 150) + "...";
